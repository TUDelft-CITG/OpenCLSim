...
class ProduceAmountActivity(GenericActivity):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.WIDprocessor = WIDprocessor
        self.amount = amount
    ...
    def main_process_function(self, activity_log, env):
        ...
        msg = (
            f"Processor {self.WIDprocessor.name} is at: "
            f"{self.WIDprocessor.geometry.wkt}."
            f"But we expect to shift an amount from the origin location ({self.origin.name}) at: "
            f"{self.origin.geometry.wkt}."
        )
        assert self.WIDprocessor.is_at(self.origin), msg
        ...
        amount = self.WIDprocessor.determine_processor_amount(
        ...
        while not all_available and amount > 0:
            ...
            yield from self._request_resource(
                self.requested_resources, self.WIDprocessor.resource
            )
        ...
        if self.WIDprocessor.resource in self.requested_resources:
            self._release_resource(
                self.requested_resources, self.WIDprocessor.resource, self.keep_resources
            )
        return amount
    ...
    def _get_shiftamount_fcn(self, amount):
        if self.duration is not None:
            ...
            return partial(self.WIDprocessor.loading, amount=amount)
        elif self.phase == "unloading":
            return partial(self.WIDprocessor.unloading, amount=amount)
        else:
            raise RuntimeError(
    ...
    def make_container_reservation(self):
        self.reserved_amount = self.WIDprocessor.determine_reservation_amount(
