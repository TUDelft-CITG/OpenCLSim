...
class ConditionProcessMixin:
    ...
    def main_process_function(self, activity_log, env):
        ...
        while True:
            ...
            if available_water_level > float(input(maintained_water_level)):
                self.register_processes()

                register_processes(self.sub_processes)
            else:
                print("the port is not accessible")
...
class WhileAccessActivity(GenericActivity, ConditionProcessMixin, RegisterSubProcesses):
    """
...
class RepeatAccessActivity(
    GenericActivity, ConditionProcessMixin, RegisterSubProcesses
):
    """
    Activity for executing multiple activities in a dedicated order within a simulation.
    ...
    def __init__(self, sub_processes, repetitions: int, show=False, *args, **kwargs):
        ...
        self.condition_event = [
            {"type": "activity", "state": "done", "accessible": False, "name": self.name}
        ]
