...
class Movable2(SimpyObject, Locatable):
    """
    Movable class.
...
class ContainerDependentMovable2(Movable2, HasContainer):
    """
    ...
    def __init__(self, compute_v_t, compute_v_s, *args, **kwargs):
        """Init of the containerdependent moveable."""
        super().__init__(*args, **kwargs)
        self.compute_v_t = compute_v_t
        self.compute_v_s = compute_v_s

    @property
    def v_t(self):
        return self.compute_v_t(
            self.container.get_level() / self.container.get_capacity()
        )
    
    def v_s(self):
        return self.compute_v_s(
            self.container.get_level() / self.container.get_capacity()
        )
...
class MultiContainerDependentMovable2(Movable2, HasMultiContainer):
    """
    ...
    def v_t(self):
        ...
        return self.compute_v_t(fill_degree)
    def v_s(self):
        sum_level = 0
        sum_capacity = 0
        for id_ in self.container.container_list:
            ...
            sum_capacity = self.container.get_capacity(id_)
        fill_degree = sum_level / sum_capacity
        return self.compute_v_s(fill_degree)
