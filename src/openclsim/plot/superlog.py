...
class SuperLog:
    ...
    def __init__(self, df_super_log):
        """dummy docstring"""
        assert set(self.COLUMNS_SUPERLOG).issubset(
            set(df_super_log.columns)
        ), "df_super_log input is missing columns"
    ...
    def _get_dependencies(self):
        ...
        for cp_act in self.df_super_log.loc[:, "cp_activity_id"].unique():
            ...
            bool_end_when_start = (
                self.df_super_log.loc[:, "end_time"]
            ...
            if sum(bool_dependencies) > 0:
                ...
                if "WAITING" in dependencies.loc[:, "state"].tolist():
                    ...
                    dependencies = self.df_super_log.loc[
    ...
    def make_gantt_mpl(self, incl_legend=True):
        ...
        for idx_object, sim_object in enumerate(
            self.df_super_log.loc[:, "SourceObject"].unique()
        ):
            bool_selection = self.df_super_log.loc[:, "SourceObject"] == sim_object
            for idx, activity in enumerate(
                self.df_super_log.loc[bool_selection, "Activity"].unique()
            ):
                # maybe combine activity with state?
                color = DEF_COLORS[color_count % len(DEF_COLORS)]
                color_count += 1
                bool_selection_activity = (
                    self.df_super_log.loc[:, "Activity"] == activity
                ) & bool_selection
                ax.barh(
                    sim_object,
                    self.df_super_log.loc[bool_selection_activity, "duration"],
                    left=self.df_super_log.loc[bool_selection_activity, "start_time"],
                    color=color,
                    label=activity,
                )
                if "is_critical" in list(self.df_super_log.columns):
                    bool_plot_critical = bool_selection_activity & (
                        self.df_super_log.loc[:, "is_critical"]
                    )
                    if sum(bool_plot_critical) > 0:
                        ...
                        for crit_act in idx_activity:
                            ax.plot(
                                self.df_super_log.loc[crit_act, "start_time"],
                                idx_object,
                                "*k",
                            )
    ...
    def make_gantt_plotly(self, static=False):
        ...
        for source_object in self.df_super_log.loc[:, "SourceObject"].unique():
            ...
            for idx, activity in enumerate(
                ...
                start_times = self.df_super_log.loc[
                    bool_selection_activity, "start_time"
                ].tolist()
                end_times = self.df_super_log.loc[
                    bool_selection_activity, "end_time"
                ].tolist()
        ...
        if "is_critical" in list(self.df_super_log.columns):
            x_critical = self.df_super_log.loc[
                self.df_super_log.loc[:, "is_critical"], "start_time"
            ].tolist()
            y = self.df_super_log.loc[
                self.df_super_log.loc[:, "is_critical"], "SourceObject"
            ].tolist()
            traces.append(
                go.Scatter(
                    name="critical_path",
        ...
        layout = go.Layout(
            ...
            xaxis=dict(
                ...
                range=[
                    min(self.df_super_log.loc[:, "start_time"]),
                    max(self.df_super_log.loc[:, "end_time"]),
                ],
            ),
            yaxis=dict(
                title="Activities",
...
def reshape_superlog(super_log):
    ...
    """
    super_log = super_log.sort_values(
    ...
    while len(to_handle) > 0 and safety_valve < len(super_log):
        ...
        bool_candidates = (
            (super_log.loc[:, "Activity"] == row_current.loc["Activity"])
                ...
                pd.DataFrame(
                    {
                        ...
                        "duration": super_log.loc[idx_end, "Timestamp"]
                        - row_current.loc["Timestamp"],
                        "end_time": super_log.loc[idx_end, "Timestamp"],
                    },
                    index=[0],
                ),
            ],
